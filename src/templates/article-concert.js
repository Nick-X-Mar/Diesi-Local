...
const ArticleConcertTemplate = props => {
    ...
    const handleLike = reaction => {
        if (userLiked && userLiked.hasOwnProperty(post.postID) && userLiked[post.postID] === reaction) {
            ...
            axios
                .post(`${process.env.GATSBY_API_URL}/likes/article/${post.postID}`, { reaction: reaction })
                .then(res => {
                    if (res.status === 200 && res.data.status === 'success') {
    ...
    return (
        <Layout location={props?.location}>
            ...
            >
                ...
                <script type="application/ld+json">
                    {`{
                        ...
                        "@type": "ListItem",
                        "position": 2,
                        "name": "Diesi In Concert",
                        "item": "${process.env.GATSBY_SITE_URL}/diesi-in-concert"
                        },{
                        "@type": "ListItem",
                        "position": 3,
                        "name": "${post.title}",
                ...
                {post?.is_video?.length > 1 && (
                    <script type="application/ld+json">
                        {`{
                            ...
                            "logo": {
                                "@type": "ImageObject",
                                "url": "${process.env.GATSBY_API_URL}/logo.jpg",
                                "width": "316",
                                "height": "215"
                            }
                            }
            ...
            <div className="article-back in-concert">
                <GatsbyImage style={{ height: '100%' }} image={imageData} alt={post.image_alt} />
            </div>
            <div className="Article page in-concert">
                ...
                <div className="more">
                    <div className="title">Δείτε προηγούμενες Συνεντεύξεις</div>
...
export default ArticleConcertTemplate;

export const pageQuery = graphql`
    query SiteConcertArticlesBySlug($slug: String!, $category: String!, $image: String!) {
